...
mod utils_test {
    use card_game::{
        enums::MAX_CARD_SIZE, test::generate_random_deck_json, utils::{deckcode_to_cards, parse_json_to_deck_code}
    };
...
#[cfg(test)]
mod game_test {
    use actix_web::{dev::Server, web::{self, Data}};
    use card_game::{
        server::{
            end_point::handle_mulligan_cards, jsons::MulliganMessage, types::{ServerState, SessionKey}
        },
        test::{generate_random_deck_json, initialize_app},
        utils::parse_json_to_deck_code, zone::zone::Zone,
    };
    use serde_json::json;
    ...
    async fn test_mulligan_reroll_restore_variants() -> std::io::Result<()> {
        ...
        async fn run_mulligan_case(reroll_count: usize) -> std::io::Result<()> {
            ...
            // mulligan 을 통해 뽑혀진 카드가 Deck 에서 제대로 제거가 됐는지 확인합니다.
            {
                ...
                if deal_cards.iter().any(|uuid| game.get_player().get().get_deck().get_cards().contains(uuid.clone()))
                {
            ...
            let rerolled_cards = if let Some(Ok(Message::Text(text))) = ws_stream.next().await {
                if let MulliganMessage::RerollAnswer(data) = serde_json::from_str::<MulliganMessage>(&text)
        ...
        for reroll_count in (1..=5).rev() {
            println!("Testing mulligan with {} card(s) being rerolled.", reroll_count);
