...
use super::{
    battlefield::{position_in_bounds, BattlefieldSize},
    types::{
    ...
    for (index, entry) in timeline.entries.iter().enumerate() {
        match entry.event {
            TimelineEvent::UnitSpawned {
                ...
                if config.validate_positions_within_bounds {
                    if let Some(size) = battlefield.as_ref() {
                        if !position_in_bounds(position, size.width, size.height) {
                            violations.push(TimelineViolation {
                                kind: TimelineViolationKind::PositionOutOfBounds,
                                message: format!(
                                    "UnitSpawned.position out of bounds: x={} y={} (width={} height={})",
