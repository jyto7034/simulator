...
mod tests {
    ...
    use crate::game::battle::core::movement::{
        ActionState, ContinuousPosition, TILE_UNITS_PER_TILE,
    };
    ...
    fn sync_unit_to_battlefield_tile_center(core: &mut BattleCore, unit_id: UnitInstanceId) {
        ...
        let center = ContinuousPosition::tile_center(position);
        if let Some(unit) = core.units.get_mut(&unit_id) {
            unit.pos_x_units = center.x_units;
            unit.pos_y_units = center.y_units;
        }
    }
    ...
    fn area_reuse_cast_target_preserves_unit_identity_for_anchor_sampling() {
        ...
        core.units.get_mut(&target_id).unwrap().pos_x_units += 250_000;
        ...
        assert_eq!(
            anchor.x_units,
            core.units.get(&target_id).unwrap().pos_x_units
        );
        assert_eq!(
            anchor.y_units,
            core.units.get(&target_id).unwrap().pos_y_units
        );
    }
    ...
    fn instant_circle_area_delivery_hits_only_units_inside_radius() {
        ...
        assert_eq!(center, ContinuousPosition::tile_center(Position::new(2, 1)));
            ...
            ActiveSkillCast {
                ...
                impact_contexts_by_step: std::iter::once((
                    ...
                    super::types::SkillImpactContext {
                        ...
                        impact_time_ms: 0,
                        impact_position: ContinuousPosition::tile_center(Position::new(2, 1)),
                        direction_hint: Some(ContinuousPosition::tile_center(Position::new(4, 1))),
    ...
    fn impact_context_before_step_does_not_fall_back_to_later_step_context() {
        ...
        let later_context = super::types::SkillImpactContext {
            ...
            impact_time_ms: 25,
            impact_position: ContinuousPosition::tile_center(Position::new(4, 1)),
            direction_hint: Some(ContinuousPosition::tile_center(Position::new(5, 1))),
    ...
    fn instant_area_delivery_can_anchor_on_previous_impact_context() {
            ...
            ActiveSkillCast {
                ...
                impact_contexts_by_step: std::iter::once((
                    ...
                    super::types::SkillImpactContext {
                        ...
                        impact_time_ms: 0,
                        impact_position: ContinuousPosition::tile_center(Position::new(3, 1)),
                        direction_hint: Some(ContinuousPosition::tile_center(Position::new(5, 1))),
        ...
        assert_eq!(center, ContinuousPosition::tile_center(Position::new(3, 1)));
