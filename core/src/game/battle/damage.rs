...
pub fn calculate_damage(request: &DamageRequest, ctx: &DamageContext) -> DamageResult {
    ...
    let base_damage = match request.source {
        DamageSource::BasicAttack => request
            .base_damage
            .saturating_sub(ctx.target_defense)
            .max(1),
        // Non-basic sources may legitimately deal 0 damage (e.g., pure buffs).
        _ => request.base_damage.saturating_sub(ctx.target_defense),
    };
