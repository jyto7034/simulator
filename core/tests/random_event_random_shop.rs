...
fn random_event_shop_inner_metadata_resolves_to_random_shop() {
    ...
    // inner_metadata 를 통해 실제 상점 메타데이터로 해석
    let target = event
        .inner_metadata
        .resolve(&game_data)
        .expect("inner_metadata resolution must succeed");

    // Then: RandomEventTarget::Shop 으로 해석되고,
    //       해당 상점이 random_shops.ron 에 정의된 상점이어야 함
    match target {
...
fn random_event_bonus_inner_metadata_resolves_to_random_bonus() {
    ...
    // suspicious_box 랜덤 이벤트는 RandomEventType::Bonus 여야 함
    let event = game_data
        .random_event_data
...
fn random_event_suppress_inner_metadata_resolves_to_random_abnormality() {
    ...
    // cursed_fountain 랜덤 이벤트는 Suppress 타입이어야 함
    let event = game_data
        .random_event_data
