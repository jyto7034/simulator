...
fn replayer_requires_expected_ability_cast_decision() {
    ...
    let timeline = Timeline {
        ...
        entries: vec![
            ...
            // Basic attack outcome is present...
            TimelineEntry {
                time_ms: 100,
                seq: 5,
                cause_seq: Some(4),
            ...
            // ...but the triggered AbilityCast decision is missing.
            TimelineEntry {
                time_ms: 200,
                seq: 6,
                cause_seq: None,
...
#[test]
fn replayer_requires_expected_buff_applied_decision() {
    let attacker_id = Uuid::from_u128(0xA);
    let target_id = Uuid::from_u128(0xB);
    let base_uuid = Uuid::from_u128(0x1000);
    let item_uuid = Uuid::from_u128(0x2000);
...
#[test]
fn replayer_can_validate_unit_base_uuid_when_enabled() {
    let game_data = empty_game_data();
    let replayer = TimelineReplayer::new(
        game_data,
        TimelineReplayerConfig {
            validate_unit_base_uuid: true,
            ..TimelineReplayerConfig::default()
        },
    );

    let base_uuid = Uuid::from_u128(0xDEAD_BEEF);
    ...
    let timeline = Timeline {
        ...
        entries: vec![
            ...
            TimelineEntry {
                ...
                cause_seq: None,
                event: TimelineEvent::UnitSpawned {
                    unit_instance_id: Uuid::from_u128(0xA),
                    owner: Side::Player,
                    base_uuid,
                    position: Position::new(0, 0),
                    stats: spawn_stats(),
