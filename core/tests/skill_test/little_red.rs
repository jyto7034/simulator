...
use super::{
    attack_starts_caused_by, hp_changes_caused_by, passive_dummy_patch, run_abnormality_scenario,
...
fn little_red_follows_marked_shot_with_two_finish_attacks_on_the_same_prey() {
    ...
    assert_eq!(
        target_unit_ids(&attack_starts_caused_by(result.timeline(), steps[1].seq)),
        vec![enemy, enemy]
    );
}
