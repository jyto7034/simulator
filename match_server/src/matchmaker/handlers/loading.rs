...
use crate::matchmaker::{
    actor::Matchmaker, lock::DistributedLock, messages::HandleLoadingComplete,
...
pub(super) fn handle_loading_complete(
    ...
    Box::pin(async move {
        ...
        let lock = match DistributedLock::acquire(&mut redis, &lock_key, LOCK_DURATION_MS).await {
            ...
            Ok(None) => {
                info!(
                    "Could not acquire lock for session {}, another process is handling it. Scheduling short retry...",
        ...
        // Loading duration metric
        // no loading duration histogram
