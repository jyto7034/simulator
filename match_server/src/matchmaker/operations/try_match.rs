...
use crate::{
    matchmaker::{operations::with_redis_timeout, scripts, MatchmakerDeps},
...
pub async fn publish_battle_request(
    ...
    let subscriber_count = with_redis_timeout("publish_battle_request", timeout_secs, async {
        redis.publish(channel, json).await
    })
    .await?;

    info!("publish_battle_request ended");
    Ok(subscriber_count)
}
