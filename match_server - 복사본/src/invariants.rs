...
pub async fn emit_violation(redis: &mut ConnectionManager, code: &str, details: serde_json::Value) {
    ...
    crate::metrics::MetricsCtx::new().inc_state_violation();
}

/// Convenience helper to emit violation with simple key-values.
pub async fn emit_violation_kv(redis: &mut ConnectionManager, code: &str, kv: &[(&str, String)]) {
