    ...
    Box::pin(async move {
        ...
        match queue_rt::add_player(&mut redis, &queue_key, &player_id_str).await {
            Ok(true) => {
                ...
                // metrics: newly enqueued player (not requeue)
                metrics.inc_enqueued_new();
                // by-mode counter
                metrics.inc_enqueued_by_mode(&msg.game_mode);
            ...
            Ok(false) => {
                ...
                // metrics: duplicate enqueue attempt
                metrics.abnormal_duplicate_enqueue();
                notify::notify_player(
                    &mut redis,
                    msg.player_id,
                    ServerMessage::Error {
