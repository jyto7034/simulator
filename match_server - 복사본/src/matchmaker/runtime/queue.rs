        ...
        Ok(added) => {
            if added > 0 {
                ...
                if let Ok(size) = redis.scard::<_, usize>(queue_key).await {
                    ...
                    let _ = emitter.queue_size_changed(game_mode.clone(), size).await;
                    // update gauge for this mode - use shared instance when available
                    let mctx = crate::metrics::MetricsCtx::new();
                    mctx.set_queue_size_for(&game_mode, size as i64);
                }
                Ok(true)
            } else {
                Ok(false)
...
pub async fn requeue_players(
    ...
    queue_key: &str,
    player_ids: &[String],
    metrics: &crate::metrics::MetricsCtx,
) {
    ...
    match inv.invoke_async::<_, (i64, i64, i64)>(redis).await {
        Ok((before_i, added_i, after_i)) => {
            ...
            // metrics: count how many players were actually re-enqueued
            if added > 0 {
                metrics.inc_requeued_by(added as u64);
            }
            ...
            {
                let mut emitter = StateEventEmitter::new(redis);
                if let Err(e) = emitter.queue_size_changed(game_mode.clone(), after).await {
                    warn!("Failed to publish queue_size_changed after requeue: {}", e);
                }
                let mctx = crate::metrics::MetricsCtx::new();
                mctx.set_queue_size_for(&game_mode, after as i64);
            }
        ...
        Err(e) => {
            ...
            match redis.sadd::<_, _, i32>(queue_key, player_ids).await {
                Ok(added) => {
                    // metrics: count how many players were actually re-enqueued (fallback path)
                    if added > 0 {
                        metrics.inc_requeued_by(added as u64);
                    }
                    // no global queue gauge adjustments
