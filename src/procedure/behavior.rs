use crate::{card::card::Card, enums::{CardSpec, InsertType, PlayerType, SpellType, TargetCard, ZoneType}, exception::exception::Exception, game::game::Game, server::{respone::RESPONE_QUEUE, schema::Respones}};
...
pub enum Behavior {
    ...
    ModifyCardSpec(CardSpec, TargetCard),
...
pub fn check_trigger(current_task_card: Card, app: &mut Procedure) -> Result<(), Exception>{
    for trigger in app.trigger_tasks.iter(){
        ...
        if has_intersection(&trigger_beheviors, &current_task_card.get_behavior_table()){
            ...
            let pos = app.tasks.iter().position(|it| it.get_task().get_data_as_card() == current_task_card);
            if let Some(pos) = pos{
...
fn modify_card_spec(card: &Card, game: &mut Game, data: &CardSpec, target: &TargetCard){
    let _player = game.get_player(card.get_player_type());

}

// Behavior::RemoveCardFromDeck => todo!(),
fn remove_card_from_zone(game: &mut Game, zone_type: ZoneType, player_type: PlayerType, card: Card){
    let player = game.get_player(player_type);
    player.as_ref().borrow_mut().get_zone(zone_type).remove_card(card.get_uuid().clone());
}

// AddCardToZone
fn add_card_to_zone(game: &mut Game, zone_type: ZoneType, player_type: PlayerType, card: Card, slot_id: i32){
    let player = game.get_player(player_type);
    player.as_ref().borrow_mut().get_zone(zone_type).add_card(card, InsertType::Slot(slot_id));
}