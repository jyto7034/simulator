//! Builds the synthetic defense objective unit required by protect-unit missions.
//!
//! Defense objectives are scenario-only units. They are not inventory items,
//! recruitable employees, or abnormalities that can be granted to the player.
...
use crate::game::{
    ...
    battle::{
        ...
        types::{
            BattleUnitDraft, BattleUnitSource, BattleUnitThreatClass, DeploymentAffinity,
            UnitCombatProfile,
        },
    },
...
pub fn defense_object_group_for_win_condition(
    ...
    let unit_ref = match win_condition {
        ...
        | WinCondition::ProtectUnitUntil { unit_ref, .. } => unit_ref,
        _ => return None,
    };
