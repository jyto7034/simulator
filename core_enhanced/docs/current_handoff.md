...
- Runtime recruitment candidates: `game_resources/data/employees/recruitment_candidates.ron`.
...
- Shop/Reward/Support minimum routing. The old Event Shop/Reward wrapper has been removed from live flow.
- HeadquartersContact safe node routing: exposes recruit/supplies/HQ shop options, uses separate `recruitment_candidates.ron`, and consumes exactly one HQ action per visit.
...
- HeadquartersContact node actions: recruit one employee, request emergency supplies, or open the headquarters supply shop.
...
- `selection.rs`: also owns `HeadquartersContactSessionState`.
...
- `WinCondition::ProtectUnit` is now the default black-box/device defense objective. The protected unit is spawned by the scenario as `BattleUnitRole::DefenseObject`, cannot be moved by the player, cannot move, cannot attack, and losing it ends the battle in the opponent's favor. Defense succeeds through the normal required-wave cleanup contract: required enemies defeated while the `DefenseObject` survives.
...
- Current implementation note: `CombatNodeType::Defense` has a core default that converts battlefield routes into tactical points, keeps the shared route end as `black_box_recovery`, injects the fixed `black_box_recovery_device` defense object, uses `FixedDefense` for player movement, routes enemies with `PathAlongPath`, and assigns `ProtectUnit(black_box_recovery_device)`.
- Newly agreed Defense direction: replace that default with an Arknights-style fixed defense contract. Defense player units do not move or redeploy during combat. Enemy waves follow explicit `route_id`s. Ground employees can block enemies by `block_radius`/`block_capacity`; platform employees cannot block. Over-capacity enemies pass through and attack the `DefenseObject` at the route end. Defense succeeds only when all waves are over, required enemies are defeated, and the `DefenseObject` survives; it fails when the `DefenseObject` is destroyed.
- G1 route overlay contract is implemented in preview/static data and runtime spawn flow: battlefield templates can define `routes`, `CombatPreview` exposes ordered route cells, `PveWaveData` can reference `route_id`, and Defense spawn waves are rejected if they do not reference a known route. Enemy spawn groups now carry route-derived `EnemyMovementPlan::PathAlongPath`; spawned runtime enemies keep that per-unit movement override, so each wave follows its authored route instead of relying only on the scenario-wide enemy plan.
- G2/G3 profile and deployment-zone contracts are partially implemented: `UnitCombatProfile` now carries `deployment_affinity`, `block_capacity`, `block_radius_units`, and `blockable`; default employees are ground-only blockers with capacity 1, abnormality/corroded enemies are blockable with no player deployment capacity, and `DefenseObject` is non-blockable with zero capacity. Battlefield ASCII `P` maps to ground deployment and `T` maps to platform deployment; `CombatPreview.deployment_zones` exposes the zone kind. `MoveUnit` and stale `ConfirmEnterNode` validation reject employees placed on incompatible zone kinds.
- G4 fixed player movement is implemented: `PlayerMovementPlan::FixedDefense` is the default Defense player plan, bypasses player group movement objectives, creates no movement/approach goal toward far enemies, and still emits an attack goal for enemies already in range. `Encirclement` continues to use `HoldDeployment`.
- G5 logical block runtime is implemented for `FixedDefense`: `BattleCore` maintains explicit blocker/enemy maps, assigns blockable enemies inside `block_radius_units` to the nearest available player blocker up to `block_capacity`, uses unit-id tie breaks, movement-locks blocked enemies, makes blocked enemies target their blocker, and makes blockers prefer their blocked enemy. Over-capacity route enemies keep following their path instead of incidental melee engagement. `PlatformOnly` unit profiles are normalized to zero block capacity/radius at runtime spawn, so platform employees cannot become blockers through bad data.
- G6 route-end targeting is implemented for the `FixedDefense + PathAlongPath` runtime: a route enemy that reaches the final tactical point targets the protected `DefenseObject`; before that endpoint it ignores incidental bystander targets unless actually blocked. The timer-based protected-unit Defense contract has been removed from runtime/data contracts.
- G9 cleanup removed the old Defense duration/cleanup API and the time-survival protected-unit schema/runtime branch. Default Defense now converts battlefield route cells into `PathAlongPath` tactical points instead of using a single `PathToPoint` destination. Wave `route_id` selection is preserved through `ScenarioSpawnGroup.enemy_movement_plan` and `RuntimeUnit.enemy_movement_plan`, and the movement planner prefers that per-unit override before falling back to the scenario-wide enemy plan. Defense routes must share one endpoint because the current contract has one protected `DefenseObject`; different endpoint Defense is future multi-objective work, not an implicit fallback.
- Newly agreed route authoring direction: battlefield terrain remains the ASCII source of truth, and route layers are same-sized ASCII overlays containing only arrows and start/end markers. Wave data must explicitly reference `route_id`; do not infer the route from spawn marker alone.
...
- Live `defend_black_box_relay` is the first authored `CombatNodeType::Defense` encounter. It uses a `ChokePoint` battlefield with `black_box_breach_main` route overlay, leaves `tactical_plan`/`win_condition` empty to exercise the transitional core default black-box defense contract, and spawns generated corroded employee waves from `black_box_breach_probe` and `black_box_breach_pressure`.
...
- Phase-era live flow has been removed. `WaitingPhaseRequest`, `SelectingEvent`, `RequestPhaseData`, `SelectEvent`, `AdvancePhase`, `RandomEventState`, `CurrentPhaseEvents`, and `GameProgression` are no longer part of the core runtime.
- The old `Event` live flow has been removed. `MapNodeCategory::Event`, `MapNodePayload::Event`, live `RandomEventDatabase` loading, `event_random`, `event_abnormality_room`, and related tests are no longer part of the official runtime.
- Future random events should be rebuilt as a new choice-driven `RandomEvent` node: situation text, 2-3 explicit options, option costs/risks/rewards, result, then map return. Do not reuse the removed Shop/Reward wrapper.
- Map content nodes now use the `shop_pool_id` and `reward_pool_id` declared in `node_definitions.ron`. These pools live on the corresponding Shop/Reward databases, so map nodes no longer draw from the full database by accident.
...
- `defense_combat_node_smoke_exports_timeline` is the Defense-specific world-flow export. It verifies Defense preview route/deployment data, node-scoped deployment, `ConfirmEnterNode`, `CombatResolved`, and writes `timeline_exports/defense_combat_node_smoke.json`.
...
- Unity-facing command results now avoid empty placement/organization responses. `MoveUnit` returns the current node `combat_deployment`, and `MoveBenchUnit` returns the full bench slot list. `SupportState` and selected support snapshots include `maintenance_options` when the resolved support type is `Maintenance`; this lists currently executable equipment restoration, dismantle, enhancement, and skill-fragment dismantle candidates. `RetreatCombat` is exposed through the `/game` WebSocket request enum.
...
- Implemented retreat policy: non-boss combat can be aborted through `RetreatCombat`. Retreat consumes the current node as failed, returns to `ViewingMap`, grants no core reward or research progress, and does not incapacitate or traumatize employees by itself. Boss battles hide the retreat capability and forced calls return `InvalidAction`.
...
- Non-combat Reward/Support/HeadquartersContact/Shop nodes must not grant employee experience. Employee XP belongs to combat participation/post-battle progression, not safe-node rewards. If a future `RandomEvent` grants growth, decide it explicitly instead of inheriting old Event behavior.
