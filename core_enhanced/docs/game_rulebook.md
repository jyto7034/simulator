...
- `Support Node`는 Safe Node 중 core의 `SupportState`를 쓰는 하위 계열이며, `Medical`, `Rest`, `Maintenance`만 포함한다.
...
- `Maintenance` 노드에서만 가능한 파편 강화/개화/분쇄, 장비 복원/분해/강화는 Safezone에 노출하지 않는다.
...
- 침식 직원 중심 `DefenseRoute` 전투는 일반 장비 파편, 손상된 부품, 엔케팔린을 제공한다.
- 정예 `DefenseRoute` 변형은 고급 장비 파편과 낮은 확률의 완제품 장비를 제공할 수 있다.
...
- Safezone의 간단 정비와 구분한다. Safezone에서는 장비 장착/해제와 파편 장착/해제만 다루며, 강화/개화/분쇄/복원/분해/강화는 `Maintenance` 노드 전용이다.
...
- 장착/해제는 `ViewingMap`, Safezone, 전투/Boss 노드의 `NodeConfirm`처럼 노드 진입 전 준비 구간에서만 가능하다.
- 전투, 노드 내부, 보상 처리 중에는 전투 결과를 보고 즉석으로 파편을 바꾸지 않는다.
- 같은 환상체가 여러 번 존재하는 보상 구조는 피한다. 성장 재료의 기본값은 동일 파편 복수 획득이 아니라 같은 rarity의 다른 파편이다.
- 같은 rarity 재료 파편은 강화와 개화 진행도에 쓰이며, 장착 중인 파편과 마지막 1개만 남은 파편은 재료로 쓰지 않는다.
